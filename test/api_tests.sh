...
CURL_OPTS="-s -X POST -H 'Content-Type: application/json'" # Common curl options for POST requests
CURL_GET_OPTS="-s -X GET -H 'Content-Type: application/json'" # Common curl options for GET requests
...
# In a real scenario, these would be generated by cryptographic libraries.
...
function register_user() {
    ...
    curl $CURL_OPTS ${BASE_URL}/auth/register --data @- << EOF | jq .
{
    "name": "$name",
    "username": "$username",
    "password": "$password",
...
function login_user() {
    ...
    RESPONSE=$(curl $CURL_OPTS ${BASE_URL}/auth/login --data @- << EOF
{
    "username": "$username",
    "password": "$password"
}
...
function add_key() {
    ...
    RESPONSE=$(curl $CURL_OPTS -H "Authorization: Bearer $token" ${BASE_URL}/keys/addKey --data @- << EOF
{
...
function get_all_keys() {
    ...
    curl $CURL_GET_OPTS -H "Authorization: Bearer $token" ${BASE_URL}/keys/getAllKeys | jq .
}
...
function fetch_public_key() {
    ...
    curl $CURL_OPTS -H "Authorization: Bearer $token" ${BASE_URL}/keys/fetchPublicKey --data @- << EOF | jq .
{
...
function delete_key() {
    ...
    curl $CURL_OPTS -H "Authorization: Bearer $token" ${BASE_URL}/keys/deleteKey --data @- << EOF | jq .
{
    "key_uid": "$key_uid"
}
EOF
...
function get_user_list() {
    ...
    curl $CURL_GET_OPTS -H "Authorization: Bearer $token" ${BASE_URL}/message/sendTo | jq .
}
...
function send_message() {
    ...
    curl $CURL_OPTS -H "Authorization: Bearer $token" ${BASE_URL}/message/send --data @- << EOF | jq .
{
    "recipient_username": "$recipient_username",
    "encrypted_content": "$message_content",
    "recipient_key_uid": "$recipient_key_uid",
    "sender_signature": "$sender_signature",
    "sender_public_identity_key_uid": "$sender_identity_key_uid"
}
EOF
}
...
function get_inbox() {
    ...
    curl $CURL_GET_OPTS -H "Authorization: Bearer $token" ${BASE_URL}/message/inbox | jq .
}
...
function get_message_by_id() {
    ...
    curl $CURL_OPTS -H "Authorization: Bearer $token" ${BASE_URL}/message/getMessageById --data @- << EOF | jq .
{
...
function delete_message() {
    ...
    curl $CURL_OPTS -H "Authorization: Bearer $token" ${BASE_URL}/message/inbox/deleteMessage --data @- << EOF | jq .
{
    "message_uid": "$message_uid"
}
EOF
...
echo "Sender Access Token: ${SENDER_ACCESS_TOKEN}"
if [ -z "$SENDER_ACCESS_TOKEN" ]; then echo "ERROR: Sender login failed."; exit 1; fi

echo ""
echo "--- 4. Recipient Login ---"
RECIPIENT_ACCESS_TOKEN=$(login_user "$RECIPIENT_USERNAME" "$RECIPIENT_PASSWORD")
echo "Recipient Access Token: ${RECIPIENT_ACCESS_TOKEN}"
if [ -z "$RECIPIENT_ACCESS_TOKEN" ]; then echo "ERROR: Recipient login failed."; exit 1; fi
...
if [ -z "$SENDER_IDENTITY_KEY_UID" ]; then echo "ERROR: Sender identity key upload failed."; exit 1; fi
...
if [ -z "$SENDER_EPHEMERAL_KEY_UID" ]; then echo "ERROR: Sender ephemeral key upload failed."; exit 1; fi
...
if [ -z "$RECIPIENT_IDENTITY_KEY_UID" ]; then echo "ERROR: Recipient identity key upload failed."; exit 1; fi
...
if [ -z "$RECIPIENT_EPHEMERAL_KEY_UID" ]; then echo "ERROR: Recipient ephemeral key upload failed."; exit 1; fi
...
curl $CURL_OPTS -H "Authorization: Bearer $SENDER_ACCESS_TOKEN" ${BASE_URL}/auth/logout | jq .

echo ""
echo "--- 20. Recipient Logs Out ---"
curl $CURL_OPTS -H "Authorization: Bearer $RECIPIENT_ACCESS_TOKEN" ${BASE_URL}/auth/logout | jq .
